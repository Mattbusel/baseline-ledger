"""Baseline Ledger Pro: the one in-app purchase, and the switch from paid to free.
...
PRODUCT_ID = "com.mattbusel.baselineledger.pro"
NAME = "Baseline Ledger Pro"
PRICE = "4.99"
DESCRIPTION = "The full stat book, every racquet, spreadsheet export."  # 55 chars max
REVIEW_NOTE = (
    "Non-consumable, one-time unlock. The app is free; practice logging, the journal, match logging, "
    "the last-ten-matches averages, goals and one racquet with its string history are free forever. Pro "
    "unlocks the full stat book on the Stats tab (first serve trend, winners and errors, results by surface, "
    "serve placement, stroke consistency, error mix, practice mix), tracking more than one racquet, and "
    "spreadsheet export. To see the paywall: open the Stats tab and tap See Baseline Ledger Pro, or tap + on "
    "the Gear tab once one racquet exists, or tap the lock on the Export the ledger card at the bottom of the "
    "Stats tab. Restore purchase is on the paywall and on the Pro card at the bottom of the Home tab."
)
